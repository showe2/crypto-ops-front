...
export default function TwitterPanel() {
  ...
  useEffect(() => {
    ...
    ref.current = setInterval(() => {
      const s = DEMO[Math.floor(Math.random() * DEMO.length)];
      const e = {
        id: `${Date.now()}`,
        phrase: s.phrase,
        source: s.source,
  ...
  return (
    <div className="grid gap-4">
      <div className="flex items-center gap-2">
        ...
        <Badge>Tier A: {tiers["A"] || 0}</Badge>
        <Badge color="yellow">Tier B: {tiers["B"] || 0}</Badge>
        <Badge>Verified: {(ver?.verifiedAccounts || []).length}</Badge>
      </div>
